<script lang="ts">
	...
	const hydrateFromServer = () => {
		...
		loading = authState === 'needs-session';
		syncingSession = authState === 'needs-session';
	};
	...
	const upcomingBookings = $derived(
		confirmedBookings
		.filter((record) => !isPast(record.appointmentDate))
		.sort(compareAsc)
	);

	const todayBookings = $derived(upcomingBookings.filter((record) => isToday(record.appointmentDate)));
...
<div class="min-h-screen bg-[linear-gradient(160deg,#f5f0ea_0%,#eee6dc_52%,#f8f4ee_100%)] pb-20 text-[#2C302E] selection:bg-[#8F9E91] selection:text-white">
	<div class="pointer-events-none absolute inset-x-0 top-0 h-72 bg-[radial-gradient(circle_at_top,_rgba(201,169,154,0.18),_transparent_58%)]"></div>
	...
	<header class="sticky top-0 z-20 border-b border-white/50 bg-[#f7f1eb]/85 backdrop-blur-xl">
		<div class="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 md:px-8">
			<div class="flex items-center gap-3">
				<div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#8F9E91] text-lg font-semibold text-white shadow-[0_12px_30px_rgba(143,158,145,0.22)]">
					五
				</div>
				<div>
					<p class="text-[11px] uppercase tracking-[0.28em] text-[#8E857D]">Admin Room</p>
					<h1 class="font-serif text-xl font-semibold tracking-[0.04em]" style="font-family: 'Playfair Display', serif;">
						五十郎 檔案室
					</h1>
				</div>
			</div>
			...
			<div class="flex items-center gap-3">
				{#if profile?.pictureUrl}
					<img src={profile.pictureUrl} alt="管理員頭像" class="hidden h-10 w-10 rounded-full border border-white/70 object-cover shadow-sm sm:block" />
				{/if}
				<div class="rounded-full border border-[#d9cec3] bg-white/80 px-3 py-2 text-right shadow-sm">
					<p class="text-[11px] tracking-[0.2em] text-[#9b9086]">
						{authState === 'authorized' ? '已驗證' : authState === 'forbidden' ? '拒絕存取' : '驗證中'}
					</p>
	...
	<main class="relative mx-auto mt-6 max-w-6xl px-5 md:px-8">
		{#if authState === 'needs-session' && loading}
			<div class="rounded-[32px] border border-white/60 bg-white/70 px-6 py-16 text-center shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:fade>
				<div class="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-[#ece2d8]">
					<div class="h-7 w-7 animate-spin rounded-full border-[3px] border-[#8F9E91] border-t-transparent"></div>
				</div>
		...
		{:else if authState === 'forbidden'}
			<div class="rounded-[32px] border border-[#ecd6d2] bg-white/80 px-6 py-14 shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:scale={{ duration: 250 }}>
				<div class="mx-auto flex max-w-lg flex-col items-center text-center">
					<div class="mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-[#f8ebe8] text-[#b08080]">
						<svg class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
							<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-7.938 4h15.876c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L2.33 16c-.77 1.333.192 3 1.732 3z" />
						</svg>
					</div>
					...
					<div class="mt-6 rounded-2xl border border-[#eadfd4] bg-[#f9f4ef] px-4 py-3 text-left">
						<p class="text-[11px] uppercase tracking-[0.24em] text-[#9b9086]">LINE User ID</p>
						<p class="mt-1 break-all font-mono text-sm text-[#5c554f]">{data.lineUserId}</p>
					</div>
				</div>
			</div>
		{:else}
			<div class="grid gap-4 lg:grid-cols-[1.45fr_0.95fr]">
				<section class="rounded-[30px] border border-white/60 bg-[linear-gradient(145deg,rgba(255,255,255,0.74),rgba(246,239,232,0.88))] p-6 shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:fly={{ y: 18, duration: 350, easing: cubicOut }}>
					<p class="text-[11px] uppercase tracking-[0.28em] text-[#9b9086]">Today Overview</p>
					<div class="mt-3 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
						<div>
							<h2 class="font-serif text-[30px] leading-tight text-[#453f3a]" style="font-family: 'Playfair Display', serif;">
								今天的營運節奏，一眼就看清楚
							</h2>
						...
						<div class="rounded-2xl border border-[#e2d7cb] bg-white/80 px-4 py-3 text-sm text-[#6a625b] shadow-sm">
							<p class="text-[11px] uppercase tracking-[0.2em] text-[#9b9086]">自動更新</p>
				...
				<section class="rounded-[30px] border border-white/60 bg-[linear-gradient(165deg,rgba(255,255,255,0.78),rgba(239,233,226,0.92))] p-6 shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:fly={{ y: 24, duration: 400, easing: cubicOut }}>
					<p class="text-[11px] uppercase tracking-[0.28em] text-[#9b9086]">Next Guest</p>
					{#if nextBooking}
						<div class="mt-4 rounded-[26px] border border-[#e4d8cd] bg-white/80 p-5 shadow-sm">
							<div class="flex items-start justify-between gap-4">
								<div>
									<p class="text-xs tracking-[0.22em] text-[#9b9086]">{getDayLabel(nextBooking.appointmentDate)}</p>
									<h3 class="mt-2 text-2xl font-semibold text-[#4c4640]">{nextBooking.customerName}</h3>
									<p class="mt-1 text-sm text-[#786f68]">{nextBooking.serviceType}</p>
								</div>
								<span class="rounded-full border border-[#dde6d8] bg-[#f3f7f1] px-3 py-1 text-[11px] font-semibold tracking-[0.18em] text-[#71806f]">
									下一位
								</span>
							</div>
							...
							<div class="mt-5 grid gap-3 sm:grid-cols-2">
								<div class="rounded-2xl bg-[#f8f4ef] px-4 py-3">
									<p class="text-[11px] uppercase tracking-[0.2em] text-[#9b9086]">到店時間</p>
									<p class="mt-2 text-lg font-semibold text-[#4c4640]">{formatTime(nextBooking.appointmentDate)}</p>
								</div>
								<div class="rounded-2xl bg-[#f8f4ef] px-4 py-3">
									<p class="text-[11px] uppercase tracking-[0.2em] text-[#9b9086]">預估結束</p>
					...
					{:else}
						<div class="mt-4 rounded-[26px] border border-dashed border-[#ddd2c7] bg-white/65 p-6 text-center text-[#7f766f]">
							<p class="text-lg font-medium">目前沒有即將到來的預約</p>
							<p class="mt-2 text-sm leading-7">今天的時段看起來已經清空，可以稍微喘口氣。</p>
						</div>
					{/if}

					<div class="mt-4 rounded-[24px] border border-[#eadfd4] bg-[#f8f3ee]/85 p-4 text-sm leading-7 text-[#786f68]">
			...
			{#if error}
				<div class="mt-4 rounded-[24px] border border-[#ecd6d2] bg-white/80 px-5 py-4 text-sm text-[#8f5f5f] shadow-sm" in:fade>
					<p class="font-medium">自動更新暫時中斷</p>
					<p class="mt-1">{error}</p>
				</div>
			{/if}

			<section class="mt-6 rounded-[30px] border border-white/60 bg-white/74 p-6 shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:fly={{ y: 18, duration: 350, easing: cubicOut }}>
				<div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
					<div>
						<p class="text-[11px] uppercase tracking-[0.26em] text-[#9b9086]">Today Schedule</p>
				...
				{#if todayBookings.length > 0}
					<div class="mt-5 grid gap-3">
						{#each todayBookings as record (record.id)}
							<div class="rounded-[24px] border border-[#ece3d9] bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(247,241,235,0.88))] p-4 shadow-sm transition-transform duration-300 hover:-translate-y-0.5" in:fade>
								<div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
									<div class="flex items-start gap-4">
										<div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#edf4ea] text-sm font-semibold text-[#6d7d6b]">
											{record.customerName.charAt(0)}
										</div>
										<div>
											<div class="flex flex-wrap items-center gap-2">
												<h3 class="text-lg font-semibold text-[#47413c]">{record.customerName}</h3>
												<span class="rounded-full border border-[#dde6d8] bg-[#f3f7f1] px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] text-[#71806f]">
													今日
												</span>
											</div>
				...
				{:else}
					<div class="mt-5 rounded-[24px] border border-dashed border-[#ded2c6] bg-[#fbf7f2]/80 px-5 py-10 text-center text-[#7f766f]">
			...
			<section class="mt-6 rounded-[30px] border border-white/60 bg-white/74 p-6 shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:fly={{ y: 18, duration: 380, easing: cubicOut }}>
				<div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
					<div>
						<p class="text-[11px] uppercase tracking-[0.26em] text-[#9b9086]">Upcoming Queue</p>
				...
				{#if laterUpcomingBookings.length > 0}
					<div class="mt-5 grid gap-3 lg:grid-cols-2">
						{#each laterUpcomingBookings as record (record.id)}
							<div class="rounded-[24px] border border-[#ece3d9] bg-[#fcfaf7]/85 p-4 shadow-sm" in:fade>
								<div class="flex items-start justify-between gap-3">
									<div>
										<p class="text-[11px] uppercase tracking-[0.22em] text-[#9b9086]">{getDayLabel(record.appointmentDate)}</p>
										<h3 class="mt-2 text-lg font-semibold text-[#47413c]">{record.customerName}</h3>
										<p class="mt-1 text-sm text-[#7a7169]">{record.serviceType}</p>
									</div>
									<span class="rounded-full bg-[#f1ece6] px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] text-[#857c74]">
										{formatTime(record.appointmentDate)}
									</span>
								</div>
								<p class="mt-4 text-sm text-[#6d655e]">
									{formatDate(record.appointmentDate)} · {record.durationMinutes} 分鐘 · 結束約 {getEndTime(record.appointmentDate, record.durationMinutes)}
								</p>
							</div>
						{/each}
					</div>
			...
			<div class="mt-6 grid gap-6 xl:grid-cols-2">
				<section class="rounded-[30px] border border-white/60 bg-white/74 p-6 shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:fly={{ y: 20, duration: 380, easing: cubicOut }}>
					<div class="flex items-center justify-between">
						<div>
							<p class="text-[11px] uppercase tracking-[0.26em] text-[#9b9086]">History</p>
				...
				<section class="rounded-[30px] border border-white/60 bg-white/74 p-6 shadow-[0_24px_60px_rgba(74,69,64,0.08)] backdrop-blur-xl" in:fly={{ y: 20, duration: 420, easing: cubicOut }}>
					<div class="flex items-center justify-between">
						<div>
							<p class="text-[11px] uppercase tracking-[0.26em] text-[#9b9086]">Cancelled</p>
					...
					{:else}
						<div class="mt-5 rounded-[24px] border border-dashed border-[#e6d3cf] bg-[#fbf7f2]/80 px-5 py-10 text-center text-[#7f766f]">
