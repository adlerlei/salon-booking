...
- Admin 後台查看所有預約（分頁式查看、每 30 秒自動更新）
...
- **Admin 後台管理介面優化 `/admin`** ✅
  - 已改成更適合店家快速掃讀的極簡管理視圖
  - `/admin` 目前改為分頁式切換：`今日`、`接下來`、`統計`、`紀錄`
  - `今日` 分頁只保留重點數字、下一位客人、今日預約清單
  - `接下來` 分頁集中顯示未來預約，依日期分組
  - `統計` 與 `紀錄` 拆成獨立分頁，不再和主要操作資訊混在同一頁
  - 主畫面只保留重點標題、數字與預約清單，移除大部分介紹與提示文字
  - 全面 RWD 響應式設計
- **管理員權限漏洞修正 `/admin`** ✅
  - 已修正非管理員短暫看見後台內容的授權顯示漏洞
  - `/admin` 現在只有在 `authorized` 狀態下才會渲染後台資料
  - 已完成實機測試確認修復成功
- **管理後台預約統計** ✅
...
- **護髮服務** ✅
  ...
  - 護髮+剪髮+洗髮（約 1 小時）
- **公休設定功能** ✅
  - 管理員可在後台「公休」tab 設定公休日
  - 支援整天公休與指定時段公休
  - 可填寫公休原因（選填），原因會顯示給客人看
  - 整天公休：預約頁該日期 disable，顯示公休原因標籤
  - 時段公休：日期可選但標示「部分時段休息」，對應時段無法預約
  - 後端預約驗證同步檢查公休衝突，防止繞過前端提交
  - DB 新增 `closures` 資料表
- **多人預約** ✅
  - 預約時可選擇 1～4 人同行
  - 每位各自選擇服務項目
  - 系統自動加總所有服務時長作為衝突檢查依據
  - 後台顯示人數 badge 與各服務明細
  - DB 新增 `party_size`、`services_json` 欄位
- **後台 Agenda 視圖** ✅
  - 以行事曆行程格式呈現，依日期分組、依時間排序
  - 一頁滾動瀏覽今日與未來所有預約
  - Tab 結構：預約（Agenda）/ 統計 / 紀錄
- **管理員新預約通知** ✅
...
- **公休資料即時更新** ✅
  - 新增公開 API `/api/closures`，回傳今日起的公休資料
  - 前端 `onMount` 時透過 API 拉取最新公休，不再只依賴 SSR 一次性載入
  - 管理員設定公休後，客戶重新進入或返回預約頁即可看到最新狀態
- **伺服器端日期時區修正** ✅
...
- **公休日期複選** ✅
  - 後台公休設定改用自製月曆元件，支援一次選取多天
  - 選好多天後填一次原因即可批次新增，不需重複操作
  - 後端 API 同步支援 `dates` 陣列批次寫入
- **預約日期改為月曆視圖** ✅
  - 預約頁日期選擇從 7 天按鈕列改為完整月曆格式
  - 可瀏覽未來 30 天，左右切換月份
  - 整天公休日標「休」並 disable、時段公休標「部分休」提示
  - 週日自動 disable、今日外框標示、已選日期綠色高亮
  - 底部圖例說明各顏色含義
- **時間 12 小時制顯示** ✅
  - 預約時段顯示改為「上午/下午」格式（如 下午 1:00）
  - 後台公休時間下拉選單與列表同步改為 12 小時制
  - 公休時段選擇改為下拉選單，限制在營業時間 11:00-20:00 內
  - 後端 API 加上營業時間範圍驗證
- **後台版本號顯示** ✅
  - 管理後台標題旁顯示目前版本號
  - 版本號自動從 `package.json` 讀取（build time 注入），只需維護一處
...
- **後台手動管理預約**
  - 管理員可手動新增、取消、改期預約
  - 適用於電話預約、現場改期、店家代客操作等情境
- **營業規則設定化**
  - 服務項目、時長、營業時間改成可設定，不要硬寫在程式裡
  - 後續若店家調整價格、服務內容時，不需要再改程式部署
- **後台搜尋 / 篩選功能**
  - 依日期、顧客、狀態快速搜尋預約資料
  - 讓店家在預約筆數增加後，仍能快速找到指定預約紀錄
- **管理員通知優化**
  - 目前先完成新預約通知基本版
  - 後續可再評估是否需要加入取消通知、通知內容優化、通知對象設定
  - 目標是讓店家收到資訊剛好夠用，不要太吵也不要漏掉重要變動
- **管理後台店家視圖再簡化**
  - 若店家實測後仍覺得資訊太多，下一步可再把 `/admin` 收斂成只保留 `今日`
  - 其餘資訊可改成次頁或獨立入口，不和每日操作畫面混在一起
  - 核心目標是讓店家打開後台後，第一眼就知道現在要處理什麼
- **正式對外前測試流程整理**
  - LIFF 發佈狀態
  - 管理員白名單
  - 店家測試帳號驗證流程
  - 整理成正式上線前的檢查清單，避免遺漏帳號、權限或環境設定問題
...
- **預約資料 Excel / CSV 匯出**
  - 讓店家可將預約資料匯出做備份、對帳或交給其他工具處理
- **操作紀錄（Audit Log）**
  - 記錄誰在什麼時間做了什麼操作，例如新增、取消、改期、權限變更
- **加入手機日曆（.ics / Google Calendar）**
  - 讓客人或店家可把預約加入手機行事曆，減少忘記時間的機率
- **避免重複送單的更完整保護**
  - 例如短時間重複提交限制
  - 避免客人在網路延遲或重複點擊時產生兩筆相同預約
- **預約狀態擴充**
  - 例如 `done`、`no-show`、`pending`
  - 讓預約不只分成已確認 / 已取消，方便後續做更完整的管理與統計
...
- 目前不做金額 / 業績統計
- 目前不要求店家手動點擊 `completed`
- 目前以「預約來客數」與「服務項目摘要」作為後台統計基準
- `/admin` 目前以店家每日操作為主，優先讓使用者先看到「今日」與「下一位」
- 未來若仍覺得畫面複雜，優先繼續減少首頁資訊，而不是再增加說明文字或區塊
